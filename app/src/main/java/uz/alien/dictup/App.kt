...
class App : Application() {
    override fun onCreate() {
        ...
        textToSpeech = TextToSpeech(this) { status -> if (status != TextToSpeech.ERROR) textToSpeech.language = Locale.ENGLISH }
    ...
    companion object {
        ...
        fun export() {
            ...
            var lineS: String?
            while (readerS.readLine().apply { lineS = this } != null) builderS.append(lineS).append("\n")
            readerS.close()
            inputStreamS.close()
            ...
            executor.execute { database.export(wordList); handler.postDelayed({ executor.execute { if (database.load().size == 3600) app.onExported() else export() } }, 100L) }
        }
        fun load() {
            executor.execute {
                words.clear()
