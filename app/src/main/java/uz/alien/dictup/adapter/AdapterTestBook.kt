...
class AdapterTestBook(private val activity: ActivityHome, private val unitAdapter: AdapterTestUnit) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    class TestBookViewHolder(view: View) : RecyclerView.ViewHolder(view) { val binding = ItemTestBookBinding.bind(view) }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder = TestBookViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_test_book, parent, false))
    override fun getItemCount() = 6
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is TestBookViewHolder) {
            buttons[position] = holder.binding.root
            if (position == 0 && !started) { started = true; swapColor(position, App.home.getColor(R.color.light_blue), duration) }
